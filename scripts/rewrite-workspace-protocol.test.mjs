...
try {
  ...
  mkdirSync(join(root, 'node_modules', '@scope', 'blocked'), { recursive: true });
  writeFileSync(
    join(root, 'node_modules', '@scope', 'blocked', 'package.json'),
    JSON.stringify({
      name: '@scope/blocked',
      version: '9.9.9',
      exports: { './shim': './shim.js' },
    })
  );
  writeFileSync(
    ...
    JSON.stringify(
      {
        name: 'app',
        dependencies: { 'dep-a': 'workspace:*', '@scope/blocked': 'workspace:*' },
        peerDependencies: { 'dep-a': 'workspace:^' },
      },
      null,
      2
    )
  ...
  assert.equal(packed.dependencies['@scope/blocked'], '9.9.9');
